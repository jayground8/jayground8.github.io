...
이제 `internal/controller/at_controller.go`에 business logic을 추가한다. At resource를 가져오고 상태값에 따라서 로직을 수행한다. CRD의 schedule의 시간이 지나면 이제 phase를 RUNNING으로 변경하고, busybox에 CRD의 command 명령어를 실행하기 위한 pod를 name과 namespace로 가져와서 없으면 이제 pod를 생성하게 된다. pod의 상태값이 failed나 succeeded이면 이제 phase는 DONE이 된다.
...
- `return reconcile.Result{RequeueAfter: d}, nil` : d만큼 기다렸다가 다시 work queue에 들어가서 다시 reconcilation을 하게 된다.

`err := controllerutil.SetControllerReference(instance, pod, r.Scheme)`는 이제 Custom resource에 pod가 자식 관계를 맺게 되어서 custom resource가 delete되면 관계가 있는 pod도 GC되도록 하려고 설정한다.

`err = r.Status().Update(context.TODO(), instance)`는 subresource status를 enable했기 때문에 이렇게 Status만 업데이트 할 수 있다.
...
controller business logic중에 pod에 대해서 get하고 create하는게 있기 때문에, `internal/controller/at_controller.go`에 아래처럼 설정해놨다.

```go
//+kubebuilder:rbac:groups="",resources=pods,verbs=get;list;create;watch;
```

`kubectl apply -f config/crd/samples/cnat.study.jayground8_ats.yaml`으로 custom resource At을 생성하면, informer가 event를 받아서 busybox container를 생성하여 command를 수행하는 것을 확인할 수 있다.
